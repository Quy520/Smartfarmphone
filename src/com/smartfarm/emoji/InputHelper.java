...
public class InputHelper {
	...
	private static final String OSC_EMOJI_URL_PATTERN1 = "<img src=\"http://www.oschina.net/js/ke/plugins/emoticons/%1$s.gif\" alt=\"%1$s\">";
	private static final String OSC_EMOJI_URL_PATTERN2 = "<img src=\"http://www.oschina.net/js/team/grunt/dist/images/emoji_mini/people/%1$s.png\" alt=\"%1$s\">";
    ...
    public static String getHtmlEmoji(Resources res, CharSequence s) {
    	
        StringBuilder result = new StringBuilder(s);

        for (int i = 0; i < result.length(); i++) {
            int index1 = result.indexOf("[", i);
            int length1 = result.indexOf("]", index1 + 1);
            int index2 = result.indexOf(":", i);
            int length2 = result.indexOf(":", index2 + 1);
            try {
                String emojiStr = result.substring(index1 + 1, length1);
                int resId = getEmojiResId("[" + emojiStr + "]");
                if (resId > 0) {
                	result.replace(index1, length1 + "]".length(), String.format(OSC_EMOJI_URL_PATTERN1, emojiStr));
                }
            } catch (Exception e) {
            }
            try {
                String emojiStr2 = result.substring(index2 + 1, length2);
                int resId2 = getEmojiResId(":" + emojiStr2 + ":");
                if (resId2 > 0) {
                	result.replace(index2, length2 + ":".length(), String.format(OSC_EMOJI_URL_PATTERN2, emojiStr2));
                }
            } catch (Exception e) {
            	e.printStackTrace();
            }
